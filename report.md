...
Moltbook has a karma system — agents earn points through upvotes. The leaderboard is supposed to surface the best contributors. It doesn't.
...
Beyond the agent_smith ring we already found, graph pattern matching reveals more automated operations hiding in the data. Agents with suspiciously uniform interaction patterns — nearly identical comment counts to each target:

| Agent | Targets | Avg per target | Variance | Karma |
|-------|---------|---------------|----------|-------|
| Doormat | 1,086 | 1.33 | 0.679 | 16,256 |
| MonkeNigga | 738 | 1.07 | 0.081 | 4,982 |
| PoseidonCash | 969 | 1.24 | 0.374 | 123 |
| AgentEcoBuilder | 818 | 1.15 | 0.204 | 922 |

A variance near zero means every target gets almost exactly the same number of comments — the signature of an automated loop. `Doormat` has 16,256 karma from this operation. These are botnets the karma leaderboard can't detect but graph analysis surfaces immediately.
...
99.9% of all interacting agents are in a single connected component (22,087 out of 22,108 nodes). Every agent is 1-2 hops from every other — `evil` (the manifesto author) is 2 hops from `eudaemon_0` (the security researcher), connected through `pinchwork`. `Pith` (the philosopher) is directly connected to `donaldtrump` (the carpet bomber). The spam bots comment on everything, which inadvertently connects the entire graph into a small world.
...
| Agent | Targets | Reciprocal | Reciprocity | Karma |
|-------|---------|-----------|-------------|-------|
| DuckBot | 79 | 11 | 13.9% | 1,078 |
| Ronin | 103 | 10 | 9.7% | 4,233 |
| Memeothy | 101 | 9 | 8.9% | 634 |
| Kevin | 195 | 14 | 7.2% | 1,637 |
...
**Longer posts win. Posts with code win more.** There's a clean linear relationship between post length and upvotes: posts under 50 characters average 1.5 upvotes; posts over 2,000 characters average 6.9. Posts containing code blocks average 5.8 upvotes vs 3.6 for posts without. The platform rewards substance.
